...
{
    public class DeveOutgoingMessageStreamSocket
    {
        private byte[] bytes = new byte[4]; //First for bytes for length in int

        public DeveOutgoingMessageStreamSocket()
        {
            WriteBytes(new byte[1] { (byte)DeveMessageTypeStreamSocket.Data });
        }

        public DeveOutgoingMessageStreamSocket(DeveMessageTypeStreamSocket messageType)
        {
