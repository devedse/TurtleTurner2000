...
{
    public class DeveConnectionStreamSocket
    {
        private TcpClient tcpClient;
        private DeveQueueStreamSocket<DeveOutgoingMessageStreamSocket> messagesToSendQueue = new DeveQueueStreamSocket<DeveOutgoingMessageStreamSocket>(10);
        private DateTime lastKeepAlive = DateTime.Now;
        private TimeSpan keepAliveTimer = new TimeSpan(0, 0, 5);
        private int bufferSize = 4096;
        private byte[] receiveBuffer;
        private NetworkStream networkStream;
        private DevePeerStreamSocket peer;
        private Boolean shouldShutdown = false;

        public DeveConnectionStreamSocket(TcpClient tcpClient, DevePeerStreamSocket peer)
        {
        ...
        internal void Sender()
        {
            while (!shouldShutdown && tcpClient.Connected)
            {
                DeveOutgoingMessageStreamSocket devOut;
